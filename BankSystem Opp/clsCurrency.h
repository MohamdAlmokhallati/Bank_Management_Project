...
private:
    ...
    static   clsCurrency _FindByCode(string CurrencyCode)
    {
...
public:
    ...
    static clsCurrency Find(string CountryOrCode)
    {
        clsCurrency Currency = _FindByCode(CountryOrCode);

        return !Currency.IsEmpty() ? Currency : _FindByCountry(CountryOrCode);
    }

    static bool IsCurrencyExist(string CurrencyCode)
    {
        clsCurrency C1 = clsCurrency::_FindByCode(CurrencyCode);
        return (!C1.IsEmpty());
    ...
    float Exchange(float Amount,clsCurrency ChangeTo)
    {
        //change to dollar in the first then to the Currency
        return (Amount / Rate())*ChangeTo.Rate();
    }
